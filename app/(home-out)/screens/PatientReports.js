...
const PatientReports = ({ route }) => {
  const { patient } = route.params; // Get the patient object from route params
  ...
  const fetchImages = async () => {
    ...
    try {
      ...
      if (patient.patientid) {
        const response = await axios.get(`${API_SERVER_URL}/images/${patient.patientid}`);
        setImages(response.data); // Set the fetched images to state
      } else {
        Alert.alert('Error', 'Patient ID does not match Doctor ID');
      }
    } catch (error) {
  ...
  }, [route.params.patientid]); // Add dependencies to refetch when these change
  ...
  return (
    <View style={styles.container}>
      {images.length === 0 ? ( // Check if there are no images
        <Text style={styles.noImagesText}>No images found for Patient ID: {patient.patientid}</Text>
      ) : (
        <FlatList
          data={images}
